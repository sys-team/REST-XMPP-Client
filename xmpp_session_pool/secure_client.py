...
class XMPPSecureClient(xmpp.Client):
    def __init__(self,id_generator,server,port=5222,debug=['always', 'nodebuilder']):
    ...
    def getRoster(self):
        ...
        if not self.__dict__.has_key('Roster'): XMPPSecureRoster(self.id_generator).PlugIn(self)
